...
const AnalysisReport: React.FC<AnalysisReportProps> = ({ analysis, dataset }) => {
  ...
  const generateChartData = () => {
    ...
    return numericColumns.map(col => ({
      name: col,
      value: analysis.numericStats[col].mean,
  ...
  const generateCategoricalChartData = (column: string) => {
    const stats = analysis.categoricalStats[column];
    if (!stats) return [];
    
    return stats.topCategories.slice(0, 6).map((cat, index) => ({
      ...
      <div className="p-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{dataset.uploadedAt.toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
        ...
        {activeTab === 'statistics' && (
          <div className="space-y-6">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {Object.entries(analysis.numericStats).map(([column, stats]) => (
                      <tr key={column}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">{stats.mean.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">{stats.median.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">{stats.stdDev.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">{stats.min.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">{stats.max.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {Object.keys(analysis.categoricalStats).length > 0 && (
              <div>
                ...
                {Object.keys(analysis.categoricalStats).slice(0, 4).map((column) => (
                  <div key={column}>
                    ...
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <RechartsPieChart>
                          <RechartsPieChart data={generateCategoricalChartData(column)}>
                            {generateCategoricalChartData(column).map((entry, index) => (
          ...
          <div className="space-y-6">
            ...
            {analysis.correlations.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {analysis.correlations.filter(corr => Math.abs(corr.coefficient) > 0.5).map((corr, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div>
                        <span className="font-medium">{corr.column1}</span> ↔ <span className="font-medium">{corr.column2}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className={`text-sm ${corr.direction === 'positive' ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {analysis.columns.map((column) => {
                      const qualityScore = 100 - column.nullPercentage;
                      return (
                        <tr key={column.name}>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">{column.nullPercentage.toFixed(1)}%</td>
                          <td className="px-6 py-4 text-sm text-gray-900">{column.uniqueValues.toLocaleString()}</td>
